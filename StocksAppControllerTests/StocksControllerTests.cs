...
namespace Tests.ControllerTests
{
    public class StocksControllerTest
    {
        ...
        public async Task Explore_StockIsNull_ShouldReturnExploreViewWithStocksList()
        {
            ...
            StocksController stocksController = new StocksController(_finnhubService, tradingOptions, _logger);
            ...
            //Act
            IActionResult result = await stocksController.Explore(true);
        ...
        #region ShowSelectedStock

        [Fact]
        public async Task ShowSelectedStock_ShouldReturnViewComponentResult()
        {
            //Arrange
            IOptions<TradingOptions> tradingOptions = Options.Create(new TradingOptions() { DefaultOrderQuantity = 100, DefaultStockSymbol = "MSFT", SelectedStockSymbol = "APPL" });

            StocksController stocksController = new StocksController(_finnhubService, tradingOptions, _logger);

            //Act
            IActionResult result = await stocksController.ShowSelectedStock(tradingOptions.Value.SelectedStockSymbol);
